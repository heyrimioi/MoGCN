...
def work(data, in_feas, out_dir, lr=0.001, bs=32, epochs=100, device=torch.device('cpu'), a=0.4, b=0.3, c=0.3, mode=0, topn=100):
    #name of sample
    sample_name = data['Sample'].tolist()

    # make directory to save

    os.makedirs(out_dir + "res/model/AE/", exist_ok=True)
    os.makedirs(out_dir + "res/result/", exist_ok=True)
    save_model_path = os.path.join(out_dir,"res/model/AE/")
    ...
    if mode == 0 or mode == 1:
        ...
        #initialize a model
        mmae = autoencoder_model.MMAE(out_dir, in_feas, latent_dim=100, a=a, b=b, c=c)
        mmae.to(device)
        mmae.train()
        ...
        torch.save(mmae, save_model_path +'MMAE_model.pkl')

    #load saved model, used for reducing dimensions
    if mode == 0 or mode == 2:
        print('Get the latent layer output...')
        mmae = torch.load(save_model_path+'MMAE_model.pkl')
        ...
        latent_df.to_csv(save_data_path + 'latent_data.csv', header=True, index=False)

    print('Extract features...')
    extract_features(data, out_dir, in_feas, epochs, topn)
    return

def extract_features(data, out_dir, in_feas, epochs, topn=100):
    # set the directory
    ...
    for epoch in tqdm(epoch_ls):
        #load model
        mmae = torch.load(os.path.join(save_model_path,'model_{}.pkl'.format(epoch)))
        #get model variables
    ...
    topn_omics_1.to_csv(save_data_path+'topn_omics_1.csv', header=True, index=False)
    topn_omics_2.to_csv(save_data_path+'topn_omics_2.csv', header=True, index=False)
    topn_omics_3.to_csv(save_data_path+'topn_omics_3.csv', header=True, index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--outdir', '-dir', type=str, required=True, help='Output directory')
    ...
    work(Merge_data, in_feas, out_dir=args.outdir, lr=args.learningrate, bs=args.batchsize, epochs=args.epoch, device=device, a=args.a, b=args.b, c=args.c, mode=args.mode, topn=args.topn)
