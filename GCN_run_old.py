...
def predict(features, adj, sample, idx):
    ...
    res_data.to_csv(args.outdir+'res/result/GCN_predicted_data.csv', header=True, index=False)
    ...
    if args.mode == 0:
        ...
        for epoch in range(args.epochs):
            ...
            if bad_counter == args.patience:
                break
            os.makedirs(args.outdir+'/res/model/GCN/', exist_ok=True)

            #save model of this epoch
            torch.save(GCN_model.state_dict(), args.outdir+'/res/model/GCN/{}.pkl'.format(epoch))

            #reserve the best model, delete other models
            files = glob.glob(args.outdir+'res/model/GCN/*.pkl')
            for file in files:
                name = file.split('/')[-1]
        ...
        GCN_model.load_state_dict(torch.load(args.outdir+'/res/model/GCN/{}.pkl'.format(best_epoch)))
